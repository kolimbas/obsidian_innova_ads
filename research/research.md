---
tags:
  - innova
  - ads
  - research
  - hub
  - nivel-1
updated: 2026-09-21
---

# 🔬 Research

> Cambios de la plataforma, novedades de Meta, benchmarks propios. Una nota por fecha. Lo que cambia cómo trabajamos se pasa al playbook correspondiente.

← Volver a [[ADS_MASTER]]

---

| Fecha | Tema |
| --- | --- |
| [[research/2026-09-21-capacitacion\|2026-09-21]] | Capacitación: Andromeda, leads, click-to-WhatsApp, políticas de salud (22-07-2026), números de Argentina. Con fuentes. |
| [[research/2026-09-14\|2026-09-14]] | Estado de Meta Ads al arrancar: qué cambió en los últimos años y qué implica para PyMEs |

---

## Fuentes que vale la pena seguir

- Centro de ayuda para empresas de Meta (cambios oficiales, políticas).
- Biblioteca de anuncios de Meta (qué corre la competencia de cada cliente).
- Meta for Business en Instagram y el blog de Ads Manager (anuncios de funciones nuevas).
